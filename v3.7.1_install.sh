...
EOF
    ...
    _add_fw_rule "$SUB_PORT_HTTP"  "$HAS_V4" "$HAS_V6"
...
if [ ! -f "$SUB_PATH_FILE" ]; then
    ...
    if [ -n "$SUB_HOST" ]; then
        ...
        SUB_URL_HTTP="http://${SUB_HOST}:${SUB_PORT_HTTP}${SUB_PATH}"
        ...
        echo -e "\n${GRAY}订阅服务状态: ${SUB_STATUS}${PLAIN}"
        echo -e "\n${CYAN}订阅地址 (HTTPS / 自签证书)${PLAIN}:"
        echo -e "${YELLOW}${SUB_URL_HTTPS}${PLAIN}"
		echo -e ""
        qrencode -t ANSIUTF8 "${SUB_URL_HTTPS}"
        echo -e "${GRAY}客户端开启 跳过证书验证/AllowInsecure${PLAIN}\n"

        echo -e "${CYAN}订阅地址 (HTTP / 无证书)${PLAIN}:"
        echo -e "${YELLOW}${SUB_URL_HTTP}${PLAIN}"
		echo -e ""
        qrencode -t ANSIUTF8 "${SUB_URL_HTTP}"
        echo -e "${GRAY}明文传输，建议仅在可信网络环境下使用${PLAIN}\n"
    fi
fi
...
echo -e "   启动服务  : systemctl start fail2ban"
echo -e "   开机自启  : systemctl enable fail2ban"
...
do_warp_direct() {
    ...
    while true; do
        ...
        echo -e "  ${GRAY}· 支持逗号分隔批量输入，例: geosite:google,gmail.com,paypal.com,wise.com,twitter.com,whatsapp.com,whatsapp.net,dmit.io,claude.ai,anthropic.com${PLAIN}\033[K"
...
# 配置变更时自动刷新缓存；异常时保留上次有效内容，确保服务持续可用
_cached_links = None
_config_mtime = None
...
http_server  = http.server.HTTPServer(("0.0.0.0", SUB_PORT_HTTP), Handler)
...
EOF
    ...
    execute_task \
        ...
        "部署并启动订阅服务 (HTTPS: ${SUB_PORT_HTTPS} / HTTP: ${SUB_PORT_HTTP})" || exit 1
